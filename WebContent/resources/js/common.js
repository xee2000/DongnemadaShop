...
function martlist_go(page,url){
      if(!url) url="martlist.do";
...
function storelist_go(page,url){
      if(!url) url="storelist.do";
...
function MemberPictureThumb(contextPath){
    for(var target of document.querySelectorAll('.manPicture')){   
       var product_code = target.getAttribute('data-id');
      
      target.style.backgroundImage="url('"+contextPath+"/shop/getPicture.do?product_code="+product_code+"')";            
